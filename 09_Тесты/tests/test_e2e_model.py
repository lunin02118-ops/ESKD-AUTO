...
class ModelNames(SwTestCase):
    ...
    def test_M13_configuration_without_material_gets_no_material(self):
        ...
        # Снимать материал можно только в активной конфигурации: SetMaterialPropertyName2(cfg, "", "") для неактивной
        # портит документ, и SolidWorks падает при его закрытии (отчёты CXPA 20–21.09.2026, M13 в полных прогонах;
        # цикл «снять → сохранить → закрыть» падает за 1–8 повторов, с активной конфигурацией — 30 повторов без падения).
        doc.ShowConfiguration2("02")
        doc.SetMaterialPropertyName2("02", "", "")
        doc.ShowConfiguration2("00")
        for cfg in ("00", "01"):
            build.set_material(doc, "Лист 4,0 ГОСТ 19903-2015 / Ст3сп ГОСТ 14637-89", cfg)
        self.s.save(doc)
        self.s.close(doc)
        disk = self.persisted(path)
        self.assertEqual("Лист Б-ПН-НО-4,0 ГОСТ 19903-2015 / Ст3сп ГОСТ 14637-2024", V(disk, "Материал_Строка", "01"),
