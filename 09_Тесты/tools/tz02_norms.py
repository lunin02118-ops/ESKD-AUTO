...
Правит только начальник КТО (решение 15.09.2026): у остальных на папку право чтения. Значения меняются в Excel — кнопка читает файл при каждом запуске, переустановка не нужна. Строки ищутся
...
NORMS = [
    ...
    ("труба.рез", "Ширина реза лазерного трубореза", 0.5, "мм", "Предварительно 15.09.2026, уточняет начальник КТО"),
...
def norms_workbook():
    ...
    for r, text in enumerate([
        "Справочник правит только начальник КТО; у конструкторов на папку право чтения. Кнопки вкладки ЕСКД читают его при каждом запуске — "
        "после правки и сохранения значения действуют сразу у всех.",
