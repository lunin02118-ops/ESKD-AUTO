...
PROD_SECTIONS = [
    {
        "sheet": "1 Заготовка",
        "title": "1. ЦЕХ МЕТАЛЛОИЗДЕЛИЙ (Заготовительный участок: Резка, Гибка, ЧПУ-раскрой)",
        "subtitle": "Спецификация чистых заготовок на тираж (для ленточной пилы, ЧПУ-трубореза, трубогиба и лазера)",
        "columns": [("№", 4, INT, "center", None), ("Обозначение", 16, None, "center", None),
                    ("Наименование детали", 21, None, "left", None), ("Сортамент / Материал", 27, None, "left", None),
                    ("L, мм", 9, DEC1, "right", None), ("На 1", 6, INT, "right", None),
                    ("Всего, шт", 8, INT, "right", '=IF(F{r}="","",F{r}*{q})'), ("Операция", 18, None, "center", None)],
        "total": {"label": '="ИТОГО ЗАГОТОВОК НА ЗАКАЗ ("&' + QTY + '&" шт.)"', "span": 5,
                  "cells": {6: "=SUM({c})", 7: "=SUM({c})", 8: '=IF(COUNTA(B{a}:B{b})=0,"",COUNTA(B{a}:B{b})&" типоразм.")'}},
        "notes": ["Резка трубного проката выполняется ленточнопильным станком строго с охлаждением СОЖ. Погрешность по длине ±0.5 мм.",
                  "Детали из листа вырезать на лазере по чистому контуру DXF из папки 03_ЧПУ изделия.",
    ...
    },
    {
        ...
        "subtitle": "Сборочные единицы металлокаркаса, нормативы сварки и масса конструкций",
        "columns": [("№", 4, INT, "center", None), ("Обозначение узла", 17, None, "center", None),
                    ("Наименование сборочной единицы", 33, None, "left", None), ("Масса 1 шт, кг", 11, DEC2, "right", None),
                    ("На 1 изд", 8, INT, "right", None), ("Всего на тираж", 10, INT, "right", '=IF(E{r}="","",E{r}*{q})'),
                    ("Общая масса, кг", 13, DEC2, "right", '=IF(F{r}="","",D{r}*F{r})')],
        "total": {"label": "ИТОГО ГОТОВЫЙ МЕТАЛЛОКАРКАС НА ЗАКАЗ", "span": 4,
                  "cells": {5: "=SUM({c})", 6: "=SUM({c})", 7: "=SUM({c})"}},
    ...
    },
    {
        ...
        "subtitle": "Расчет потребности в хлыстах 6000 мм, КИМ, деловых обрезков (>=500 мм) и технологического отхода",
        "columns": [("№", 4, INT, "center", None), ("Сортамент и ГОСТ", 25, None, "left", None),
                    ("Чистый расход, м", 9, DEC1, "right", None), ("Хлыстов 6 м", 8, None, "right", None),
                    ("Расход с уч. техн., м", 9, DEC1, "right", None), ("Расход с уч. техн., кг", 9, DEC1, "right", None),
                    ("КИМ, %", 7, "0.0%;-0.0%;;@", "right", None), ("Деловые обрезки (>=500 мм)", 25, None, "left", None),
                    ("Отход, кг", 8, DEC1, "right", None)],
        "total": {"label": "ИТОГО МЕТАЛЛОПРОКАТ НА ЗАКАЗ", "span": 2,
                  "cells": {3: "=SUM({c})", 5: "=SUM({c})", 6: "=SUM({c})", 9: "=SUM({c})"}},
        "notes": ["Технологический отход металла (стружка, торцовка и обрезки <500 мм) подлежит сдаче в металлолом."],
    ...
    for k, values in enumerate(rows):
        ...
        for i, col in enumerate(cols, start=1):
            ...
            if isinstance(value, str) and not value.startswith("="):
                height = max(height, text_height(value, col[1] - 1, 9))
