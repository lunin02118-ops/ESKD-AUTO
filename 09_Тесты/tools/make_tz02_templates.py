...
PROD_SECTIONS = [
    ...
    },
    {
        ...
        "subtitle": "Ведомость окрашиваемых единиц по цветовым исполнениям заказа",
        # Одно изделие в заказе может идти в нескольких расцветках (часть парт бело-синие, часть жёлто-красные):
        # строка — окрашиваемая единица в одном исполнении; «Изделий» — сколько изделий этого исполнения, по умолчанию тираж.
        "columns": [("№", 4, INT, "center", None), ("Обозначение узла", 16, None, "center", None),
                    ("Наименование сборочной единицы", 24, None, "left", None), ("Цветовое исполнение", 16, None, "left", None),
                    ("Изделий", 8, INT, "right", '=IF(B{r}="","",{q})'), ("На 1 изд", 7, INT, "right", None),
                    ("Всего", 8, INT, "right", '=IF(F{r}="","",E{r}*F{r})'), ("Цвет покрытия", 18, None, "center", None)],
        "total": {"label": "ИТОГО НА ПОКРАСКУ", "span": 5, "cells": {6: "=SUM({c})", 7: "=SUM({c})"}},
...
def prod_sheet(wb, spec, data, section_data):
    ...
    for r, (left, right, right_width) in enumerate(head, start=1):
        ...
        ws[f"A{r}"].alignment = Alignment(vertical="center", shrink_to_fit=True)  # длинное наименование изделия ужимается, не обрезается
