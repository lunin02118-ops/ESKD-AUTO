...
namespace ESKD.MaterialSync.Sw
{
    ...
    public static class FormatReloadService
    {
        ...
        {
            { "MYPRP4", 44.5 }, { "MYPRP3", 26.0 }, { "MYPRP15", 38.0 }
        };
        ...
        private static void ReloadFile(ISldWorks app, string path, bool apply, Summary summary, string swplus, string root)
        {
            ...
            try
            {
                ...
                foreach (string name in names)
                {
                    ...
                    r.Geometry = geometry;
                    r.State = geometry == "" ? "форма 2 или без граф эталона" : (etalon ? "эталон" : "устарела");
        ...
        private static bool IsEtalon(DrawingDoc drw, out string geometry)
        {
            ...
            while (noteObj != null)
            {
                ...
                double wanted;
                if (EtalonY.TryGetValue(name, out wanted))
                {
                    ...
                    if (pos != null && pos.Length >= 2)
                    {
                        ...
                        sb.Append(name).Append(" y=").Append(y.ToString("0.##", CultureInfo.InvariantCulture));
