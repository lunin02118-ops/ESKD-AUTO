...
DOCS["tz02"] = dict(
    ...
    org="ТОО «Троя» · Павлодар · Конструкторско-технологический отдел",
    kicker="Техническое задание · ТЗ-02 · редакция 3", title="Кнопки вкладки ЕСКД для работы по регламенту КТО",
    title_plain="ТЗ-02. Кнопки вкладки ЕСКД",
    sub="Восемь новых кнопок надстройки: требования, спайки, этапы, тесты, приёмка. Для агента-разработчика",
    run="ТЗ-02 · Кнопки вкладки ЕСКД",
    chips=[(8, "новых кнопок", "#e7f5ec", "#1e7b45"), (55, "требований Т-1…Т-55", "#e8f1fa", "#1f5f99"),
           (7, "спайков до старта", "#fff4e0", "#a15c00")],
    foot_l="Решения заказчика от 15.09.2026: реализуй — да; таблица изменений — да; Покрытие — не нужно", foot_r="15 сентября 2026 г.", toc_depth=2,
    cards=[("Не меняется", "PropertyWriter, SyncService, BchService, правило «как MProp»; сторонних библиотек нет."),
           ("Не читается", "Служебка от продаж — для человека. Битрикс24 не используется. Цвет из модели не берётся."),
           ("Порядок", "Э0 спайки → Э1 словарь (таблица изменений) → Э2 каркас → К-1…К-3 → К-4, К-7, К-8 → К-5, К-6."),
           ("Приёмка", "U01–U08, S01–S05, E01–E08 на стенде «заказ», протокол П-2 на живом заказе.")])

DOCS["tz02plain"] = dict(
    dir="../ТЗ",
    md="ТЗ-02_Простыми_словами.md", pdf="ТЗ-02_Простыми_словами.pdf",
    org="ТОО «Троя» · Павлодар · Конструкторско-технологический отдел",
    kicker="К ТЗ-02 · для Начальника КТО", title="Что будут делать новые кнопки",
    ...
    foot_l="Техническая версия ТЗ-02 — для программиста", foot_r="15 сентября 2026 г.", toc_depth=1,
    cards=[("Ведомость", "На одно изделие, из модели, один раз. У стандартных — в базе, у новых — в заказе."),
           ("Сводная", "Список «изделие — количество» в окне; система берёт ведомости, умножает и складывает. Пять листов, как 85_Т."),
           ("История изменений", "Журнал в папке изделия пишет кнопка. Человек его не ведёт."),
...
DOCS["memos"] = dict(
    ...
    org="ТОО «Троя» · Павлодар · Конструкторско-технологический отдел",
    kicker="Памятки · редакция 1", title="Кто что делает: памятки по ролям",
    ...
    foot_l="Основание: ТЗ-02 ред. 3, концепция упрощения ред. 2", foot_r="15 сентября 2026 г.", toc_depth=2,
    cards=[("Конструктор", "Своя папка в заказе, четыре кнопки: отвязать, проверить, экспорт, ведомость."),
           ("Начальник КТО", "Задача в Битрикс24, приёмка по отчёту, «Выдать», «Закрыть». Журналов нет."),
...
DOCS["regl3"] = dict(
    ...
    org="ТОО «Троя» · Павлодар · Конструкторско-технологический отдел",
    kicker="Регламент · редакция 3.0 · действующая", title="Регламент КТО «Металл»: заказы, файлы, изменения",
    ...
    foot_l="Заменяет v2.2 от 14.09.2026", foot_r="15 сентября 2026 г.", toc_depth=2,
