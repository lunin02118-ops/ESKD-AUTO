...
DOCS = {
    "audit": dict(
        dir="03_Архив_версий/Аудиты_и_заключения",
...
# Инструкции по ролям v2.2 — архив (03_Архив_версий): карточки описывают тогдашний порядок с 04_ПРОИЗВОДСТВО.
# Действующий порядок (ТЗ-04, папки «Производство» нет) — в «Памятках по ролям» и регламенте v3.1.
ROLE_COMMON = dict(dir="03_Архив_версий/Инструкции_v2.2", org="ТОО «Троя» · Павлодар · Конструкторско-технологический отдел",
...
DOCS["sa_task"] = dict(ROLE_COMMON, dir="05_Инструкции_по_ролям", md="Задание_сисадмину_права_КТО.md", pdf="Задание_сисадмину_права_КТО.pdf",
    kicker="Задание системному администратору · редакция 3", title="Права КТО на Synology",
    title_plain="Задание сисадмину: права КТО", sub="Шесть групп и стандартные права на папки отдела один раз — без делегирования и без прав администратора NAS",
    run="Задание сисадмину: права КТО", foot_r="19 сентября 2026 г.",
    cards=[("Сделать", "Группы, права, папка _Архив, снимки нужных папок — около 40 минут"), ("Не делать", "Не включать в administrators, не делегировать"),
           ("Проверить", "Семь проверок с Начальником КТО"), ("Ответить", "Таблица из 10 вопросов")])
...
DOCS["tz02"] = dict(
    dir="../ТЗ",
    md="ТЗ-02_Кнопки_вкладки_ЕСКД_для_работы_по_регламенту.md", pdf="ТЗ-02_Кнопки_вкладки_ЕСКД_для_работы_по_регламенту.pdf",
    org="ТОО «Троя» · Павлодар · Конструкторско-технологический отдел",
    kicker="Техническое задание · ТЗ-02 · редакция 4", title="Кнопки вкладки ЕСКД для работы по регламенту КТО",
    title_plain="ТЗ-02. Кнопки вкладки ЕСКД",
    sub="Восемь новых кнопок надстройки: решения заказчика, требования, спайки, этапы, тесты, приёмка. Для агента-разработчика",
    run="ТЗ-02 · Кнопки вкладки ЕСКД",
    chips=[(8, "новых кнопок", "#e7f5ec", "#1e7b45"), (59, "требований Т-1…Т-59", "#e8f1fa", "#1f5f99"),
           (15, "решений заказчика 15.09", "#fff4e0", "#a15c00"), (6, "спайков выполнено из 10", "#f1eaf9", "#6a3fa0")],
    foot_l="Регламент v3.1 · решения заказчика от 15.09.2026 (раздел 0)", foot_r="15 сентября 2026 г.", toc_depth=2,
    cards=[("Не меняется", "PropertyWriter, SyncService, BchService, правило MProp; VBA не трогаем; сторонних библиотек нет."),
           ("Решено 15.09", "Папка 02_Металл\\И01_…; операции ставит конструктор; окно цветов с комплектами RAL; ревизия у чертежа; DXF со слоем сгибов."),
           ("Порядок", "Ш0 репозитории → Ш1 спайки → Ш2 словарь и установщик → Ш3 ядро → Ш4 каркас → К-3, К-2, К-4, К-1, К-7, К-8, К-5, К-6 → приёмка."),
           ("Приёмка", "U01–U11, S01–S07, E01–E09 на стенде «заказ», протокол П-2 на живом заказе.")])

DOCS["tz02plan"] = dict(
    dir="../ТЗ",
    md="ТЗ-02_План_реализации.md", pdf="ТЗ-02_План_реализации.pdf",
    org="ТОО «Троя» · Павлодар · Конструкторско-технологический отдел",
    kicker="К ТЗ-02 · план реализации", title="Пошаговый план реализации кнопок вкладки ЕСКД",
    title_plain="ТЗ-02. План реализации",
    sub="Двенадцать шагов: что делается, какие файлы, чем проверяется, что заказчик видит после каждого шага и что от него нужно",
    run="ТЗ-02 · План реализации",
    chips=[(12, "шагов Ш0…Ш11", "#e7f5ec", "#1e7b45"), (4, "спайка до кода", "#fff4e0", "#a15c00"),
           (8, "–9 недель чистой работы", "#e8f1fa", "#1f5f99")],
    foot_l="К ТЗ-02 ред. 4 и регламенту v3.1", foot_r="15 сентября 2026 г.", toc_depth=2,
    cards=[("Сначала", "Закрыть репозиторий, закоммитить SWTools, четыре спайка, ответы на пять вопросов."),
           ("Первая польза", "«Проверить изделие» и «Экспорт» — после шага Ш6, через 3–4 недели."),
           ("Каждый шаг", "Коммит, зелёные тесты, файл или экран заказчику на просмотр."),
           ("Риски", "SWTools без командной строки, склейка PDF, скорость на сети, схема шифров — с обходными путями.")])

DOCS["tz02plain"] = dict(
    dir="../ТЗ",
    md="ТЗ-02_Простыми_словами.md", pdf="ТЗ-02_Простыми_словами.pdf",
    org="ТОО «Троя» · Павлодар · Конструкторско-технологический отдел",
    kicker="К ТЗ-02 · для Начальника КТО · редакция 4", title="Что будут делать новые кнопки",
    title_plain="ТЗ-02 простыми словами",
    sub="Восемь кнопок вкладки ЕСКД: кто нажимает, что получается, что настраивается, чего не будет",
    run="ТЗ-02 · простыми словами",
    chips=[(8, "кнопок", "#e7f5ec", "#1e7b45"), (2, "уровня документов: ведомость и сводная", "#e8f1fa", "#1f5f99"),
           (0, "связей с Битрикс24", "#fff4e0", "#a15c00")],
    foot_l="Техническая версия ТЗ-02 ред. 4 — для программиста", foot_r="15 сентября 2026 г.", toc_depth=1,
    cards=[("Ведомость", "На одно изделие, из модели, один раз; перед ней — окно «Операции». У стандартных — в базе, у новых — в заказе."),
           ("Сводная", "Список «изделие — количество», окно цветов с комплектами RAL, нормативы на заказ. Пять листов, как 85_Т."),
           ("История изменений", "Ревизия у каждого чертежа; журнал в папке изделия пишет кнопка. Человек его не ведёт."),
           ("Приёмка", "Живой заказ: конструктор — 10 минут вместо часа, технолог сверяет строки, мастер открывает файлы.")])

DOCS["memos"] = dict(
    dir="05_Инструкции_по_ролям",
    md="Памятки_по_ролям_простыми_словами.md", pdf="Памятки_по_ролям_простыми_словами.pdf",
    org="ТОО «Троя» · Павлодар · Конструкторско-технологический отдел",
    kicker="Памятки · редакция 2", title="Кто что делает: памятки по ролям",
    title_plain="Памятки по ролям простыми словами",
    sub="Один пример заказа на всех. Найдите свою роль — одна–две страницы. Кнопки вкладки ЕСКД появляются по мере готовности",
    run="Памятки по ролям",
    chips=[(9, "ролей", "#e7f5ec", "#1e7b45"), (1, "сквозной пример: заказ № 52", "#e8f1fa", "#1f5f99"),
           (10, "кнопок вкладки ЕСКД", "#fff4e0", "#a15c00")],
    foot_l="Основание: регламент v3.1, ТЗ-02 ред. 4", foot_r="15 сентября 2026 г.", toc_depth=2,
    cards=[("Конструктор", "Папка 02_Металл\\И01_… в заказе, четыре кнопки: отвязать, проверить, экспорт, ведомость с операциями."),
           ("Начальник КТО", "Задача в Битрикс24, приёмка по отчёту, «Выдать» с окном цветов, «Закрыть». Журналов нет."),
           ("Куратор", "Единственный, кто пишет в базу. Снимок → правка → ревизия."),
           ("Цех и склад", "Только папка производства, только последняя ревизия, только сводная заявка.")])

DOCS["regl3"] = dict(
    dir="03_Архив_версий",
    md="РЕГЛАМЕНТ_КТО_v3.0_2026-09-15.md", pdf="РЕГЛАМЕНТ_КТО_v3.0_2026-09-15.pdf",
    org="ТОО «Троя» · Павлодар · Конструкторско-технологический отдел",
    kicker="Регламент · редакция 3.0 · архив", title="Регламент КТО «Металл»: заказы, файлы, изменения",
    title_plain="Регламент КТО v3.0",
    sub="Как устроена работа отдела на сетевом диске с SolidWorks и вкладкой ЕСКД. Только правила и решения; что делают кнопки — в ТЗ-02, кто что нажимает — в памятках",
    run="Регламент КТО v3.0",
    chips=[(5, "принципов", "#e7f5ec", "#1e7b45"), (3, "состояния изделия: черновик · выдано · сдано", "#e8f1fa", "#1f5f99"),
           (0, "журналов, которые ведёт человек", "#fff4e0", "#a15c00")],
    foot_l="Заменяет v2.2 от 14.09.2026; заменён v3.1", foot_r="15 сентября 2026 г.", toc_depth=2,
    cards=[("Где лежит", "Z: — библиотека, база, заказы, производство. Y: — архив. Пишут по ролям, читают все."),
           ("Как работает", "Черновик правится свободно. Выдано — только через новую ревизию. Сдано — в архив."),
           ("Стандартное", "База пишется куратором. Изменение — снимок, правка, ревизия, применяемость."),
           ("Что автоматизировано", "Восемь кнопок вкладки ЕСКД по ТЗ-02; ведомость — SWTools.")])

DOCS["regl31"] = dict(DOCS["regl3"],
    dir="01_Действующая_редакция",
    md="РЕГЛАМЕНТ_КТО_v3.1_2026-09-15.md", pdf="РЕГЛАМЕНТ_КТО_v3.1_2026-09-15.pdf",
    kicker="Регламент · редакция 3.1 · действующая", title_plain="Регламент КТО v3.1", run="Регламент КТО v3.1",
    foot_l="Заменяет v3.0 от 15.09.2026: решения владельца от 15.09 (вечер)",
    cards=[("Где лежит", "Z: — библиотека, база, заказы (02_Металл\\И01_…), производство. Y: — архив, удаляет только Начальник КТО."),
           ("Как работает", "Черновик правится свободно. Выданный чертёж — только через новую ревизию. Сдано — в архив."),
           ("Стандартное", "База пишется куратором, всем остальным — только чтение. Изменение — снимок, правка, ревизия, применяемость."),
           ("Что автоматизировано", "Восемь кнопок вкладки ЕСКД по ТЗ-02 ред. 4: операции, цвета, нормативы из справочника; ведомость — SWTools.")])
