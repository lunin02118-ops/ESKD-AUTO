...
> **ЗАМЕНЁН — не применять** (аудит 19.09.2026, М-К1). PDF по структуре заказа выгружает кнопка «Экспорт для производства» надстройки ЕСКД (`ExportService`: `02_PDF`, `_ИзмN`, старые — в «Аннулировано»). Патч в SaveAsPDF.swp не вносился; внесённый, он дал бы второй способ выгрузки с другим архивированием.
